...
public class Tablero extends Lista<Lista<Casilla>> {
	...
	//public void setCas(Casilla newCas,int x, int y){
		...
		//oldCas=newCas ;
	//}
	public boolean leer(String fc,int filaocol){ /*esta funcion lee una fila
												o columna y devuelve false si encuentra 
												una palabra que no está en el diccionario,
		...
		if (fc == "f"){  //revisa una fila
			while(posCasilla<14){
				for (int i=posCasilla;i<15;i++){// quita las casillas vacias
				...
				for (int i=posCasilla;i<15;i++){ //lee las letras que estan juntas
					letra=this.getCas(i,filaocol).getFicha().getLetra();
					posCasilla=i;
					if (letra!=null){
				...
				if ( largoLectura>1){  //revisa que la palabra este en el diccionario
			...
			while(posCasilla<14){ 
				...
				if (! lectura.equals("")){ //revisa que la palabra este en el diccionario
					if(! dicc.isDicc(lectura)){ ///!!!!!!!!!!!!!!!!!!!!!!!//
						return false;
					}
					lectura="";
